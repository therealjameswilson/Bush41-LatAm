...
function printLedgerSourceFields(candidate) {
  ...
  return {
    sourceFamily: source.name || sourceName(candidate),
    repository: "George H.W. Bush Library",
    recordGroupOrCollection: source.name || sourceName(candidate),
    series: source.name || sourceName(candidate),
...
function primaryCountry(countries) {
  ...
  return [...(countries || [])]
    ...
    .sort((a, b) => countryRank(a) - countryRank(b))[0] || "Regional/No single country";
}
...
function annotationRow({
  ...
  return {
    ...
    anchor_chapter_country: anchorRecord.chapter?.name || primaryCountry(countryList(anchorRecord)),
...
function annotationRegisterRows({ audit, memcons, printCandidates, publicStatements }) {
  const riskMap = riskByCountry(audit);
  const rows = [];
    ...
    if (/partial/i.test(record.releaseStatus || "")) {
      rows.push(
        annotationRow({
          ...
          contextSource: sourceShort(record),
    ...
    for (const item of nearbyPrintCandidates(record, printCandidates, 14).filter((lead) => lead.candidate.priority === "High").slice(0, 8)) {
      ...
      rows.push(
        annotationRow({
          ...
          contextSource: sourceName(candidate),
  ...
  return rows
    .sort(
      (a, b) =>
        ...
        countryRank(a.anchor_chapter_country) - countryRank(b.anchor_chapter_country) ||
...
function sourceLedgerRows({ audit, memcons, printCandidates, dailyDiaryReferences }) {
  ...
  for (const reference of dailyDiaryReferences) {
    addSourceLedgerEvidence(ledger, diaryLedgerSourceFields(reference), {
      ...
      countries: countryList(reference),
      date: reference.date || "",
      title: reference.title || "",
...
function issueDossierRows({ audit, memcons, printCandidates, dailyDiaryReferences, publicStatements }) {
...
function main() {
  ...
  const exports = [
    ...
    writeCsv(
      "annotation-register.csv",
      annotationRegisterRows({
        audit: compilerGaps,
        memcons,
        printCandidates,
        publicStatements
